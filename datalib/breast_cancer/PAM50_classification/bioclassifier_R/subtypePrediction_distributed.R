## This R script was modified based on orginal subtypePrediction_distributed.R from PAM50_subtypePrediction
...
PAM50_subtypePrediction <- function(paramDir=paramDir,
	...
	pamout.centroids<-read.table(trainCentroids,sep="\t",header=T,row.names=1,check.names=F)
		...
		if(hasClinical){
			...
			dimnames(outtable)[[2]]<-c("Basal","Her2","LumA","LumB","Normal-like","Call","Confidence",
		...
		subtypeColors[subtypeColors=="Normal-like"]<-"green"
